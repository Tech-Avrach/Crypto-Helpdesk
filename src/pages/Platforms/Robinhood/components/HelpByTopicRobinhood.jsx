...
export default function HelpByTopicRobinhood() {
  const topics = [
    {
      id: 1,
      title: "Getting Started",
      icon: Clock,
      description: "Create your account and explore Robinhood features.",
      iconBg: "bg-green-600",
    },
    {
      id: 2,
    ...
    },
    {
      id: 6,
      title: "Privacy & Data",
      description: "Learn how Robinhood keeps your data secure.",
      icon: Shield,
      iconBg: "bg-green-600",
    },
    {
    ...
    },
    {
      ...
      description: "Understand market trends and Robinhood analytics tools.",
      icon: BarChart3,
      iconBg: "bg-green-600",
    }
  ];
  ...
  const TopicCard = ({ topic }) => {
    ...
    return (
      ...
      >
        ...
        <motion.div
          className="absolute inset-0 bg-gradient-to-br from-green-50/30 via-transparent to-gray-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
          initial={false}
        />
        
        {/* Decorative Floating Glow */}
        <motion.div
          className="absolute -top-8 -right-8 w-16 h-16 bg-gradient-to-br from-green-100/50 to-green-200/30 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-700"
        ...
        <div className="relative z-10 mb-6">
          ...
          >
            <IconComponent 
              className={`w-8 h-8 ${topic.iconBg === 'bg-white' ? 'text-green-600' : 'text-white'} group-hover:scale-110 transition-transform duration-300`} 
            />
          </motion.div>
        </div>
        ...
        {/* Border Highlight */}
        <motion.div
          className="absolute inset-0 border border-green-200/50 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"
          initial={false}
        />
      </motion.div>
    );
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-16 bg-white">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          className="absolute top-1/4 left-1/4 w-2 h-2 bg-green-400/30 rounded-full"
          animate={{ y: [0, -20, 0], opacity: [0.3, 0.8, 0.3] }}
          transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute top-1/2 right-1/3 w-3 h-3 bg-green-400/20 rounded-full"
