...
export default function TopCashAppIssues() {
  ...
  const rightColumnIssues = [
    ...
    { title: "Cash App features", hasArrow: false, isCategory: true },
    { title: "Cryptocurrency wallets on Cash App", hasArrow: true },
  ...
  const IssueItem = ({ issue, index }) => (
    <motion.div
      ...
      className={`group cursor-pointer transition-all duration-300 ${
        issue.isCategory 
          ? 'py-4 border-b border-[#00e011] ' 
          : 'py-4 px-4 rounded-xl border border-gray-100 hover:border-[#00e011] hover:bg-[#00e011]/30 hover:shadow-lg hover:shadow-[#00e011]/50 mb-3 bg-white'
      }`}
    ...
    >
      <div className="flex items-center justify-between">
        <span className={`${ 
          issue.isCategory 
            ...
            : 'text-gray-700 group-hover:text-[#00e011] font-medium'
        } transition-colors duration-300`}>
          {issue.title}
        </span>
        {issue.hasArrow && (
          <div className="relative">
            <ChevronRight className="w-5 h-5 text-gray-400 group-hover:text-[#00e011] group-hover:translate-x-2 transition-all duration-300" />
            <motion.div
              className="absolute inset-0 bg-[#00e011] rounded-full -z-10"
      ...
      {!issue.isCategory && (
        <motion.div
          className="absolute inset-0 bg-gradient-to-r from-[#00e011]/5 via-transparent to-[#00e011]/5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
          initial={false}
        />
      )}
    </motion.div>
    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-16 bg-white">
      ...
      >
        <h2 className="text-3xl md:text-4xl font-light text-gray-800 mb-2">
          Top{' '}
          <span className="font-black text-[#00e011]">
            Cash App
          </span>
          {' '}
          Issues
